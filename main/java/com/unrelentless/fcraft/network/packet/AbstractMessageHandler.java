package com.unrelentless.fcraft.network.packet;
...
public abstract class AbstractMessageHandler<T extends IMessage> implements IMessageHandler <T, IMessage>
{
	...
	public IMessage onMessage(T message, MessageContext ctx) {
		if (ctx.side.isClient()) {
			return handleClientMessage(FantasyCraft.proxy.getPlayerEntity(ctx), message, ctx);
		} else {
			return handleServerMessage(FantasyCraft.proxy.getPlayerEntity(ctx), message, ctx);
		}
	}
}